...
export default function AccusedDetailPage() {
  ...
  const [roleInCrime, setRoleInCrime] = useState("");

  const role = Cookies.get("role");
  const accusedId = params.id;

  async function loadAccused() {
    try {
      const result = await fetchAPI(`/accused/${accusedId}`);
      setData(result.data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleDelete = async () => {
    if (!confirm("Are you sure you want to delete this accused record?")) return;
    try {
      await fetchAPI(`/accused/${accusedId}`, { method: "DELETE" });
      router.push("/accused");
    } catch (e: any) {
  ...
  const handleUnlink = async (firAccusedId: number) => {
    if (!confirm("Are you sure you want to unlink this FIR?")) return;
    try {
      await fetchAPI(`/accused/unlink-fir/${firAccusedId}`, { method: "DELETE" });
      await loadAccused();
    } catch (e: any) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      await fetchAPI(`/accused/${accusedId}`, {
        method: "PATCH",
  ...
  const openLinkModal = async () => {
    ...
    setLoadingFirs(true);
    try {
      // Fetch open/active FIRs only to avoid massive lists, or just fetch all
  ...
  const submitLink = async () => {
    ...
    try {
      await fetchAPI("/accused/link-fir", {
        method: "POST",
        body: JSON.stringify({
          fir_id: selectedFir,
          accused_id: parseInt(accusedId as string),
          role_in_crime: roleInCrime || null
        })
      });
  ...
  if (loading) return <div className="p-8 text-center text-slate-400">Loading accused details...</div>;
  if (!data) return <div className="p-8 text-center text-red-500">Accused not found.</div>;

  const { linked_firs, ...accused } = data;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "Cleared": return <Badge variant="outline" className="border-green-600 text-green-500">Cleared</Badge>;
    ...
    <div className="p-8 space-y-6 max-w-5xl mx-auto relative">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white flex items-center gap-3">
              ...
              {getStatusBadge(accused.status)}
            </h1>
            <p className="text-slate-400 mt-1">Accused ID: #{accused.accused_id}</p>
          </div>
        </div>
          ...
          <CardContent className="space-y-4 text-sm pt-2">
            <div>
              ...
              {(role === "Admin" || role === "Officer" || role === "Detective") ? (
                ...
                <span className="font-medium text-white">{accused.status}</span>
              )}
            </div>
            <div>
              <span className="text-slate-500 block mb-1">Age / Gender</span>
              <span className="font-medium text-white">{accused.age} / {accused.gender}</span>
            </div>
            <div>
              <span className="text-slate-500 block mb-1">Nationality</span>
              <span className="font-medium text-white">{accused.nationality}</span>
            </div>
            <div>
              <span className="text-slate-500 block mb-1">Contact No</span>
              <span className="font-medium text-white">{accused.contact_no || 'N/A'}</span>
            </div>
            <div>
              <span className="text-slate-500 block mb-1">Address</span>
              <span className="font-medium text-white">{accused.address || 'N/A'}</span>
            </div>
            <div>
              <span className="text-slate-500 block mb-1">Profile Created</span>
              <span className="font-medium text-white">{new Date(accused.created_at).toLocaleString()}</span>
            </div>
          </CardContent>
        </Card>
                ...
                <Table>
                  ...
                  <TableBody>
                    {linked_firs.map((link: any) => (
                      <TableRow key={link.fir_accused_id} className="border-slate-800 hover:bg-slate-900/50">
                        <TableCell>
                        ...
                        <TableCell className="text-slate-400 text-xs">{link.role_in_crime || 'Unspecified'}</TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          {(role === "Officer" || role === "Detective" || role === "Admin") && (
                            <Button 
                              ...
                              onClick={() => handleUnlink(link.fir_accused_id)}
                            >
      ...
      {showLinkModal && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-lg max-w-3xl w-full flex flex-col max-h-[85vh] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-slate-950">
              <h2 className="text-lg font-semibold text-white">Link FIR to Accused</h2>
            ...
            <div className="p-4 overflow-y-auto flex-1 bg-slate-950/50">
              <p className="text-sm text-slate-400 mb-4">Select an FIR from the register below to link this accused individual to the case.</p>
              ...
              {selectedFir && (
                <div className="mt-6 space-y-3 animate-in slide-in-from-bottom-2">
                  <h3 className="text-sm font-medium text-white border-t border-slate-800 pt-4">Specify Role (Optional)</h3>
                  <Input 
                    placeholder="e.g. Mastermind, Accomplice, Driver..." 
